#!/usr/bin/env python3
"""Dry-run Book 3 validation for the Gray Star assistant.
...
def prime(section: int = 1, *, cs: int = 40, end: int = 140, wp: int = 160) -> DryRunAssistant:
    ...
    assistant.state["CurrentBookStats"] = {
        "BookNumber": 3,
        "BookTitle": garystar.BOOKS[3]["Title"],
...
def run_completion_and_achievements(result: Result) -> None:
    ...
    result.check(3 in garystar.as_list(local.character["CompletedBooks"]), "Success-route smoke completes Book 3.", "Book 3 did not complete on success-route smoke.")
