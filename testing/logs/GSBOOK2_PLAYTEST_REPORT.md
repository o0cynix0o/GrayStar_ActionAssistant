# GS Book 2 Playtest Report

Generated: 2026-05-26T11:03:36

Passed: 20
Warnings: 0
Failures: 0

## Passed

- All 310 Book 2 section files exist.
- Every Book 2 source link points to an existing section.
- All 310 sections are reachable from section 1 through source links.
- Book 2 automation data loads.
- Book 2 flow data loads.
- Book 1 data still loads after multi-book merge.
- All 18 detected combat sections have combat presets.
- All 9 detected roll sections have roll helpers.
- Simple automation sweep covered 130 Book 2 sections.
- Roll helper sweep covered 90 outcomes.
- Loot sweep covered 15 buttons.
- Willpower-cost sweep covered 16 buttons.
- Combat sweep started 19 presets.
- Section 99 ignores Grey Star END loss in round 1.
- Section 297 discards Backpack contents and marks Backpack unavailable.
- Capture and release store and restore Staff/Backpack gear.
- Karmo Potion use doubles END/WP, applies the side-effect roll, returns a vial, and halves after combat.
- Success-route smoke completes Book 2.
- Achievement sync/backfill unlocked expected Book 2 route set (13 new in smoke).
- Repeat Book 2 resets to section 1 and preserves achievements.

## Warnings

- None

## Failures

- None
